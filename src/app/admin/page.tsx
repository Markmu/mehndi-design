...
async function AdminWrapper() {
  const [images, tags] = await Promise.all([
    fetch(`/api/images?page=1&pageSize=20&tag=all`).then(res => res.json()),
    fetch(`/api/tags`).then(res => res.json())
  ]);

  return <AdminContent initialImages={images.data} initialTags={tags} />;
}

// 修改后的客户端组件
function AdminContent({ initialImages, initialTags }: { 
  initialImages: Image[];
  initialTags: Tag[];
}) {
